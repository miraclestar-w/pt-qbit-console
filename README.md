...
- 前端：日期切换、历史 7/14/30 日柱状图（Shift+单击对比两日）、近 N 日汇总与 Top Tracker、Tracker 搜索/排序/仅有流量、JSON/CSV 导出、健康状态、纯上传日 ∞ 高亮
...
- 上传/下载速度自适应单位
- **今日流量**（优先显示）：通过 Console `GET /api/stats/today` 拉取，Console 离线时回退全时总量
...
| `CONSOLE_URL` / `PT_CONSOLE_URL` | 小组件拉取今日统计的 Console 地址（默认 `http://127.0.0.1:8787`） |
...
单元测试覆盖：种子增量、跨日继承、大间隔丢弃、强制采样 429、ingestLive 缓存采样、墙钟秒、失败退避、今日 brief / 多日 rollup / Top Tracker。
...
| GET | `/api/stats/today` | 今日轻量摘要（小组件 / 轮询） |
| GET | `/api/stats/rollup?days=7` | 近 N 日流量汇总（峰值日、日均） |
| GET | `/api/stats/top-trackers?days=7&limit=20` | 近 N 日 Tracker 流量排行 |
